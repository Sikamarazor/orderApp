...
export class AddAddressesComponent implements OnInit {
  ...
  saveAddress() {
    ...
    if (this.phyAddress.length === 0) {
      this.operationsService.addAddress(this.uid, this.physicalAddress).then(data => {
        alert('Successfully updated');
      });
    } else {
      this.operationsService.updateAddress(this.uid, this.physicalAddress, this.addressUid).then(data => {
        alert('Successfully updated');
      });
    }
  }
